...
public class App {
    ...
    public Auction listDetailsForAuction() {
        ...
        Auction auction = restTemplate.getForObject(API_URL + "/" + scannerInput, Auction.class);
        return auction;
    }
    ...
    public Auction[] findAuctionsSearchTitle() {
    	...
    	String scannerInput = scanner.nextLine();
    	  try {
    		  auctions = restTemplate.getForObject(API_URL + "?title_like=" + scannerInput, Auction[].class);  
    	    } catch (HttpClientErrorException ex) {
    	        System.out.println("The title was not found. Please try again.");
    	    }
        return auctions;
    }
    ...
    public Auction[] findAuctionsSearchPrice() {
    	Auction[] auctions = null;
    	double scannerInput = 0;
    	  try {
    		 scannerInput = Double.parseDouble(scanner.nextLine()); 
    	    } catch (NumberFormatException e) {
    	        
    	    }
    	  auctions = restTemplate.getForObject(API_URL + "?currentBid_lte=" + scannerInput, Auction[].class); 
